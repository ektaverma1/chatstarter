...
export const list = authenticatedQuery({
    handler: async (ctx) => {
        const directMessages = await ctx.db.query("directMessageMembers").withIndex("by_user", (q) => q.eq("user", ctx.user._id)).collect();
...
export const create = authenticatedMutation({
    ...
    handler: async (ctx, {username}) => {
        ...
        const directMessagesForCurrentUser = await ctx.db.query("directMessageMembers").withIndex("by_user", (q) => q.eq("user", ctx.user._id)).collect();
        ...
        directMessagesForOtherUser.find((dm2) => dm.directMessage === dm2.directMessage))

        if(directMessage) {
            return directMessage.directMessage;
        }

        const newDirectMeesage = await ctx.db.insert("directMessages", {});
        await Promise.all([
            ctx.db.insert("directMessageMembers", {
                user: ctx.user._id,
                directMessage: newDirectMeesage
            }),
            ctx.db.insert("directMessageMembers", {
                user: user._id,
                directMessage: newDirectMeesage
...
const getDirectMessage = async (ctx: QueryCtx & {user: Doc<"users">}, id: Id<"directMessages">) => {
    ...
    const otherMember = await ctx.db.query("directMessageMembers").withIndex("by_direct_message", q => q.eq("directMessage", id)).filter((q) => q.neq(q.field("user"), ctx.user._id)).first();
